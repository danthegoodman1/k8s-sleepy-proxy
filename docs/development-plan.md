...
| Partial | 16H: Tests. | Added focused projection unit tests, real-client readiness-helper tests, wake regression coverage for unowned live refs before apply, reconciler tests for pending unowned/stale stamps plus deleting missing, owned, unowned, delete-error, lease-loss, and finalizer-blocked paths, API/sidecar transport coverage for owned-only cleanup, Ready metadata/readiness drift report-only behavior, inspect-failed observations, and force-operation projection observations, plus low-cardinality metric-label guards for object/finalizer/volume details. Existing kind E2E targets compile locally with `cargo test -p control-plane --tests --no-run`; real kind drift/finalizer injection live runs remain deferred. |
