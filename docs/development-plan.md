...
| Incomplete | Backoff and retry policies. | Sidecar idle retry/backoff, proxy Subscribe reconnect/lazy rebuild with reconnect backoff, and materializer client retry for transient Kubernetes apply/delete/PVC/readiness failures are covered; database retry remains the retry gap; follow-up: M9. |
...
| Incomplete | Retry/backoff tests cover transient database errors, Kubernetes conflicts, proxy disconnects, and materializer retries. | Proxy Subscribe transport reconnect after disconnect with reconnect backoff, stream-close invalidation/lazy rebuild, sidecar retry, Kubernetes conflict retry classification, and materializer apply/delete/PVC/readiness retry gates are covered; database retry remains the retry gap; follow-up: M9. |
