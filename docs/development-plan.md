...
| Incomplete | 6F: Failure-path matrix. | `scripts/test-kind-e2e-failures.sh` now covers full-platform wake/readiness timeout, bad route miss, missing PVC binding, bad volume template rejection, and stale proxy generation rejection through deployed production control-plane/frontline/sidecar images, real Postgres, and the real Kubernetes API. `scripts/test-kind-e2e-restart.sh` adds partial full-platform restart evidence for wake, sleep, delete, and HTTP-01 recovery, but route reassignment remains incomplete because immediate post-restart route delete/create can still serve the stale old backend from the frontline cache. |
...
| Incomplete | Control-plane restart E2E covers restart during wake, sleep, delete, route reassignment, and HTTP-01. | `scripts/test-kind-e2e-restart.sh` builds and loads production `control-plane`, `frontline`, and `sidecar` images, deploys real Postgres/control-plane/frontline, and `kind_e2e_restart.rs` proves wake retry from durable `Waking` state, sleep cleanup after `ReportIdle` during control-plane unavailability, delete cleanup retry, and HTTP-01 put/serve/resolve/delete/expiry across control-plane restart. The gate intentionally still rejects completion because route reassignment immediately after control-plane restart plus route delete/create can serve the stale old backend from frontline cache. |
...
| Incomplete | Load tests for route lookup and hot proxy path. | `scripts/smoke-frontline-load.sh`, sidecar load smokes, `crates/frontline/benches/route_lookup.rs`, and `docs/proxy-hot-path-budgets.md` exist, and the frontline smoke now enforces zero extra `SubscribeRoute` calls during the measured hot-cache HTTP/1.1 phase; protocol throughput matrix, cold-wake load coverage, and stable tail-latency gates are still missing; follow-up: M9. |
...
| Incomplete | Container tests prove images start, run non-root, include required files, access CA certs, and are used by kind E2E/soak. | `scripts/smoke-images.sh` checks non-root/no shell/files/CA/image-size/startup-error behavior, and `scripts/test-kind-e2e-stateless.sh` proves production-image startup/connectivity in stateless kind E2E; production images are still not covered by full wake/sleep soak. |
...
| Incomplete | 9AD: Full-platform control-plane restart gate. | `scripts/test-kind-e2e-restart.sh` builds and loads production `control-plane`, `frontline`, and `sidecar` images plus `postgres:17-alpine` and test workload images, deploys real Postgres/control-plane/frontline into kind, and `kind_e2e_restart.rs` uses deployed operator/proxy/frontline/sidecar paths plus the real Kubernetes API. The gate proves wake retry from durable `Waking` state after a delayed image pull and control-plane restart, sidecar sleep cleanup after control-plane unavailability, operator delete cleanup retry after control-plane unavailability, and HTTP-01 challenge put/serve/resolve/delete/expiry across restart. It intentionally remains incomplete because route reassignment immediately after control-plane restart plus route delete/create still serves the stale old backend from the frontline cache before positive TTL expiry; completing this row needs production Subscribe invalidation/update behavior for route-binding changes across restart. |
