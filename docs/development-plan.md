...
| Incomplete | 3F: HTTP/1.1, HTTP/2, h2c gRPC, and WebSocket forwarding. | Forwarding helper tests cover these paths, and the HTTP listener now accepts h2c prior-knowledge requests through route cache resolution to h2c upstreams with gRPC-shaped body/trailer preservation; WebSocket listener wiring and the full protocol matrix are still not wired end to end; follow-up: M9 protocol gates. |
...
| Incomplete | Protocol tests cover HTTP/1.1, HTTP/2, h2c gRPC, WebSockets, TLS termination, SNI passthrough, and HTTP-01. | Helper/component tests cover pieces; HTTP-01 runtime wiring is covered; the HTTP listener accepts h2c prior-knowledge gRPC-shaped requests and forwards to h2c upstreams. Runtime TLS/SNI, WebSocket listener integration, and full E2E protocol coverage remain missing; follow-up: M9. |
| Incomplete | Protocol tests include large/streaming HTTP, HTTP/2 multiplexing, gRPC trailers/status, WebSocket backpressure, TLS passthrough bytes, and TCP half-close. | Listener-level h2c gRPC body/trailer preservation, TLS passthrough bytes, and TCP half-close have partial coverage; full large/streaming/multiplex/backpressure matrix is missing; follow-up: M9. |
