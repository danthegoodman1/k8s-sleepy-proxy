...
| Incomplete | TCP tests cover byte preservation, half-close, upstream reset, client reset, timeout, and backpressure. | `tests/tcp_proxy.rs` now covers connect errors, client disconnect, upstream disconnect, drain timeout while stalled, and bounded-duplex backpressure; OS-level reset and deterministic connect-timeout coverage remain follow-up M9 protocol gaps. |
...
| Status | Item | Evidence / gap |
| --- | --- | --- |
| Incomplete | Fake-control-plane tests cover cold wake, hot route, miss, stale generation, targeted update, invalidation, and stream reconnect. | Runtime/resolver/route tests cover cold/hot/miss/stale and pushed update/invalidation; transport reconnect and manual `StreamClosed` invalidation are covered, but no runtime path turns an actual Subscribe stream close into active cache invalidation before TTL expiry. |
...
| Incomplete | Subscription tests cover duplicate in-flight SubscribeRoute, reassignment, invalidation during wake, and stream backpressure. | Reassignment, duplicate resolved responses, invalidation during wake, and response-buffer backpressure are covered; true duplicate outstanding SubscribeRoute requests for the same identity are not coalesced or proven impossible beyond the current single-`&mut` resolver API. |
| Incomplete | Stream reconnect tests prove lazy resubscribe/rebuild without public cursors. | `control_plane_transport.rs` proves a closed Subscribe stream is dropped and the next SubscribeRoute opens a fresh stream; `route.rs` proves rebuild after manual stream-loss invalidation, but actual stream close does not yet invalidate active cached subscriptions before TTL expiry. |
...
| Incomplete | Backoff and retry policies. | Sidecar idle retry/backoff and proxy Subscribe transport reconnect are covered; database retry, Kubernetes conflict retry, proxy reconnect backoff policy, active cache invalidation on stream close, and materializer reconcile retry gates are missing; follow-up: M9. |
| Incomplete | Proxy `Subscribe` reconnect and lazy cache rebuild. | `control_plane_transport.rs` covers reconnect after a closed Subscribe response stream and `route.rs` covers lazy rebuild after injected active-state loss; actual stream-close-driven active cache invalidation is missing. |
...
| Incomplete | 7C: Proxy Subscribe reconnect, lazy cache rebuild, and stale backend recovery. | Transport reconnect and stale backend recovery have component coverage, and lazy rebuild is proven after injected invalidation; actual stream-close-driven active cache invalidation before TTL expiry remains missing. |
...
| Incomplete | Retry/backoff tests cover transient database errors, Kubernetes conflicts, proxy disconnects, and materializer retries. | Proxy Subscribe transport reconnect after disconnect and sidecar retry are covered; database retry, Kubernetes conflict retry, proxy reconnect backoff policy, active cache invalidation on proxy stream close, and materializer retry gates are still missing; follow-up: M9. |
