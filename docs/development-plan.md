...
| Incomplete | Lifecycle-race E2E prevents stale sidecar, materialization, and proxy cache updates. | `scripts/test-kind-e2e-lifecycle-races.sh` passed on 2026-06-24 using production `control-plane`, `frontline`, and `sidecar` images plus real Postgres and Kubernetes API; it proves stale sidecar reports are rejected, pending materialization refs recorded during wake allow delete-while-waking cleanup of StatefulSet/Service/PVC/PV objects, delete while Draining/deleting materialization cleans StatefulSet/Service/PVC/PV objects and serves no stale backend, stale failed-wake generations cannot publish after a later successful wake, and active route subscriptions invalidate/re-resolve on reassignment before subsequent requests reach the new backend. Unit coverage now proves applied refs are best-effort deleted if pending materialization recording fails or if apply/PVC-bound wait fails before pending refs are recorded. The lifecycle-race matrix remains incomplete only for process loss or an indefinitely stuck PVC-bound wait before pending refs are recorded. |
...
| Incomplete | Load tests for route lookup and hot proxy path. | `scripts/smoke-frontline-load.sh`, sidecar load smokes, `crates/frontline/benches/route_lookup.rs`, and `docs/proxy-hot-path-budgets.md` exist, and the frontline smoke now enforces zero extra `SubscribeRoute` calls during measured hot-cache HTTP/1.1, h2c gRPC-shaped, and WebSocket phases plus one fake-control-plane cold-wake success with distinct `SubscribeRoute`, `WakeInstance`, and backend-hit counters; stable tail-latency gates and enforced regression budgets are still missing; follow-up: M9. |
...
| Incomplete | 7E: Load tests for route lookup and proxy protocols. | Conservative load smokes and the route lookup benchmark exist, and the frontline smoke now includes prior-knowledge h2c gRPC-shaped and WebSocket hot-cache direct-baseline phases plus one fake-control-plane cold-wake success path, but stable tail-latency and enforced regression budgets are incomplete; follow-up: M9. |
...
| Incomplete | Load-test targets for route lookup, hot proxy path, and cold wake latency are defined before 7D. | `docs/proxy-hot-path-budgets.md` documents route lookup and current load-smoke limitations, and `scripts/smoke-frontline-load.sh` now proves one fake-control-plane cold wake through the production frontline image, but cold-wake latency targets and stable tail-latency targets are incomplete; follow-up: M9. |
| Incomplete | Proxy load tests use production images and direct-backend baselines. | Production-image load smokes exist with fake control plane/direct comparisons for sidecar HTTP/TCP and frontline HTTP/1.1, h2c gRPC-shaped, and WebSocket hot-cache forwarding, plus one frontend cold-wake success path with separate `SubscribeRoute`, `WakeInstance`, and backend-hit assertions; stable release-budget gates are missing; follow-up: M9. |
...
| Incomplete | Hot-cache h2, h2c, and gRPC request rate stays within 25%. | `scripts/smoke-frontline-load.sh` now runs a conservative prior-knowledge h2c gRPC-shaped production-image smoke with direct baseline and zero measured hot-cache `SubscribeRoute` calls, but it is not a stable 25% release-gate budget and does not cover negotiated HTTP/2 over TLS or real generated gRPC clients; follow-up: M9. |
...
| Incomplete | WebSocket streaming throughput stays within 15-20%. | `scripts/smoke-frontline-load.sh` now performs real WebSocket upgrade and bidirectional text/binary frame load with a direct-backend baseline, but it is a conservative smoke and not a stable 15-20% streaming throughput gate; follow-up: M9. |
