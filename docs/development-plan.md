...
| Incomplete | Lifecycle-race E2E prevents stale sidecar, materialization, and proxy cache updates. | `scripts/test-kind-e2e-lifecycle-races.sh` passed on 2026-06-24 using production `control-plane`, `frontline`, and `sidecar` images plus real Postgres and Kubernetes API; it proves stale sidecar reports are rejected, pending materialization refs recorded during wake allow delete-while-waking cleanup of StatefulSet/Service/PVC/PV objects, stale failed-wake generations cannot publish after a later successful wake, and active route subscriptions invalidate/re-resolve on reassignment before subsequent requests reach the new backend. Unit coverage now proves applied refs are best-effort deleted if pending materialization recording fails. Delete-while-draining and the partial-apply/PVC-wait window before all refs are known are not yet deterministically covered, so the lifecycle-race matrix remains incomplete. |
