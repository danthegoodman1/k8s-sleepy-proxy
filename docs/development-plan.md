...
| Incomplete | Protocol tests cover HTTP/1.1, HTTP/2, h2c gRPC, WebSockets, TLS termination, SNI passthrough, and HTTP-01. | Helper/component tests cover pieces; `scripts/test-kind-e2e-routing.sh` covers full-platform HTTP/1.1 route identity and HTTP-01 challenge behavior through deployed components; the HTTP listener accepts h2c prior-knowledge gRPC-shaped requests and forwards cached ready WebSocket upgrades bidirectionally; runtime TLS termination and SNI passthrough listener wiring is covered. Full E2E protocol coverage for HTTP/2, h2c gRPC, WebSockets, TLS termination, and SNI passthrough remains missing; follow-up: M9 kind protocol gate. |
...
| Incomplete | 6C: Custom host, wildcard host, SNI, and path-prefix routing. | `scripts/test-kind-e2e-routing.sh` covers custom exact host, wildcard host, wildcard misses, and path-prefix routing including `/api`, `/api/child`, and `/apix` boundary behavior through deployed production components. SNI routing remains missing from full-platform E2E; follow-up: M9 protocol/SNI gate. |
| Incomplete | 6D: Full protocol matrix, including PostgreSQL/libpq SNI passthrough. | Helper/component tests cover pieces, including gRPC-Web operator transport; runtime TLS/SNI listener wiring is covered, but full-platform browser/kind protocol coverage and PostgreSQL/libpq SNI E2E are missing; follow-up: M9. |
...
| Incomplete | kind E2E passes for HTTP/1.1, HTTP/2, h2c gRPC, grpc-web, WebSockets, TLS termination, and SNI passthrough. | `scripts/test-kind-e2e-routing.sh` covers full-platform HTTP/1.1 route identity and HTTP-01 behavior; component gRPC-Web transport coverage exists in `api_transport.rs`. Full-platform HTTP/2, h2c gRPC, grpc-web browser path, WebSockets, TLS termination, and SNI passthrough E2E remain missing; follow-up: M9 protocol/SNI gate. |
