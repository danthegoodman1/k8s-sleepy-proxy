...
| Incomplete | Load tests for route lookup and hot proxy path. | `scripts/smoke-frontline-load.sh`, sidecar load smokes, `crates/frontline/benches/route_lookup.rs`, and `docs/proxy-hot-path-budgets.md` exist, and the frontline smoke now enforces zero extra `SubscribeRoute` calls during measured hot-cache HTTP/1.1 and h2c gRPC-shaped phases; WebSocket load coverage, cold-wake load coverage, and stable tail-latency gates are still missing; follow-up: M9. |
...
| Incomplete | 7E: Load tests for route lookup and proxy protocols. | Conservative load smokes and the route lookup benchmark exist, and the frontline smoke now includes a prior-knowledge h2c gRPC-shaped hot-cache direct-baseline phase, but WebSocket, cold-wake, tail-latency, and enforced regression budgets are incomplete; follow-up: M9. |
...
| Incomplete | Proxy load tests use production images and direct-backend baselines. | Production-image load smokes exist with fake control plane/direct comparisons for sidecar HTTP/TCP and frontline HTTP/1.1 plus h2c gRPC-shaped hot-cache forwarding, but WebSocket load, cold-wake load, and stable release-budget gates are missing; follow-up: M9. |
