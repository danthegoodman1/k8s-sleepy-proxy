...
| Incomplete | Protocol tests cover HTTP/1.1, HTTP/2, h2c gRPC, WebSockets, TLS termination, SNI passthrough, and HTTP-01. | Helper/component tests cover pieces; HTTP-01 runtime wiring is covered; the HTTP listener accepts h2c prior-knowledge gRPC-shaped requests and forwards cached ready WebSocket upgrades bidirectionally. Runtime TLS/SNI and full E2E protocol coverage remain missing; follow-up: M9. |
...
| Incomplete | Backoff and retry policies. | Sidecar idle retry/backoff, proxy Subscribe reconnect/lazy rebuild, and materializer client retry for transient Kubernetes apply/delete/PVC/readiness failures are covered; database retry and proxy reconnect backoff policy are still missing; follow-up: M9. |
...
| Incomplete | Retry/backoff tests cover transient database errors, Kubernetes conflicts, proxy disconnects, and materializer retries. | Proxy Subscribe transport reconnect after disconnect, stream-close invalidation/lazy rebuild, sidecar retry, Kubernetes conflict retry classification, and materializer apply/delete/PVC/readiness retry gates are covered; database retry and proxy reconnect backoff policy are still missing; follow-up: M9. |
