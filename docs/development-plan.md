...
| Incomplete | Protocol tests cover HTTP/1.1, HTTP/2, h2c gRPC, WebSockets, TLS termination, SNI passthrough, and HTTP-01. | Helper/component tests cover pieces; HTTP-01 runtime wiring is covered; the HTTP listener accepts h2c prior-knowledge gRPC-shaped requests and forwards cached ready WebSocket upgrades bidirectionally; runtime TLS termination and SNI passthrough listener wiring is covered. Full E2E protocol coverage remains missing; follow-up: M9 kind protocol gate. |
...
| Incomplete | kind tests prove cold wake creates PV/PVC before StatefulSet and PVC binds intended PV. | Materializer-only kind test proves apply/bind ordering; the full-platform kind E2E added in `scripts/test-kind-e2e-stateless.sh` is stateless-only and does not cover StatefulSet/PV/PVC; follow-up: M9 StatefulSet/PV/PVC full-platform E2E. |
...
| Incomplete | kind tests prove sleep deletes workload, Service, PVC, and PV. | `scripts/test-kind-e2e-stateless.sh` and `kind_e2e_stateless.rs` prove deployed sidecar `ReportIdle` deletes the stateless Deployment and Service; full-platform PVC/PV cleanup remains missing; follow-up: M9 StatefulSet/PV/PVC full-platform E2E. |
...
| Complete | Sidecar reports idle. | `kind_e2e_stateless.rs` uses a short WorkloadClass sleep policy and waits for the deployed sidecar `ReportIdle` path to return the instance to `Cold` and delete the rendered Deployment/Service. |
