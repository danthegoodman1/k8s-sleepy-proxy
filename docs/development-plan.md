...
| Complete | 7E: Load tests for route lookup and proxy protocols. | Conservative local smoke defaults and strict release-style budget knobs now exist for frontend HTTP/1.1, h2c gRPC-shaped, WebSocket sessions, sidecar HTTP/1.1, and sidecar TCP large streams, all using production images and direct-backend baselines. The route lookup benchmark remains the local indexed-cache gate. Negotiated HTTP/2-over-TLS, real generated gRPC clients, and WebSocket byte-streaming throughput remain tracked separately below rather than being implied by this row. |
...
| Incomplete | Hot-cache h2, h2c, and gRPC request rate stays within 25%. | The h2c gRPC-shaped production-image phase now has a strict direct-baseline release gate via `SLEEPYPODS_FRONTLINE_LOAD_SMOKE_GRPC_MIN_RATIO=0.75`, but negotiated HTTP/2 over TLS and real generated gRPC clients are not implemented by these load smokes; follow-up: M9/M10 protocol coverage as appropriate. |
...
| Incomplete | WebSocket streaming throughput stays within 15-20%. | The frontline WebSocket hot-cache smoke now has strict session-rate and p99 added-latency gates, but it validates short bidirectional text/binary sessions rather than byte-streaming throughput; follow-up: M9 if true WebSocket streaming throughput remains required. |
...
| Complete | 9AL: Load budget metrics and gates. | The frontline and sidecar load-smoke clients now emit stable successful operation latency fields (`p50_ms`, `p95_ms`, `p99_ms`, `max_ms`) for HTTP/1.1, h2c gRPC-shaped, WebSocket session, sidecar HTTP/1.1, and sidecar TCP stream phases. `scripts/smoke-frontline-load.sh`, `scripts/smoke-sidecar-load.sh`, and `scripts/smoke-sidecar-tcp-load.sh` use a small shared budget helper to reject missing/malformed metrics, non-positive direct baselines, request-rate/throughput ratio regressions, and p99 added-latency regressions. Relaxed local smoke defaults remain conservative, while `SLEEPYPODS_*_STRICT_BUDGETS=1` enables documented release-style defaults in `docs/proxy-hot-path-budgets.md`. |
