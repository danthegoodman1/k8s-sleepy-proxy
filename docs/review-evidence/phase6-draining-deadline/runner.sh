...
# Frozen-image diagnostic: verify existing artifacts and give only delayed
# fixture aliases fresh names. No production binary/image is rebuilt or retagged.
for component in control-plane frontline sidecar; do
  docker image inspect "${image_prefix}/${component}:${image_tag}" >/dev/null
done
for image in "${app_image}" "${postgres_image}"; do
  docker image inspect "${image}" >/dev/null
done
for image in "${sleep_waking_image}" "${delete_waking_image}" "${failed_retry_image}"; do
  docker tag "${app_image}" "${image}"
done
...
echo "==> Creating new retained diagnostic namespace ${namespace}"
if KUBECONFIG="${kubeconfig}" kubectl get namespace "${namespace}" >/dev/null 2>&1; then
  echo "Diagnostic namespace already exists; refusing to change it" >&2
  exit 2
fi
for volume in lifecycle-delete-waking-pv-0b1fe29d lifecycle-delete-draining-pv-3cf19f0e; do
  if KUBECONFIG="${kubeconfig}" kubectl get persistentvolume "${volume}" >/dev/null 2>&1; then
    echo "Fixture PV name already exists; refusing to change it: ${volume}" >&2
    exit 2
  fi
done
      ...
      containers:
        - name: control-plane
          ...
          env:
            ...
            # Keep the failed-wake deadline explicit and inside this gate budget.
