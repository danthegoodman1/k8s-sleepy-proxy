...
async fn control_plane_restart_recovery_through_deployed_platform() -> TestResult<()> {
    ...
    create_workload_class(&mut operator, NORMAL_CLASS_ID, &config.app_image, &config.sidecar_image, sleep_policy(900_000), "normal").await?;
    let result = route_reassignment_after_restart_does_not_serve_stale_backend(&mut operator, kube, &config).await;
    eprintln!("DIAGNOSTIC_ORIGINAL_RESULT {result:?}");
    let joins = std::mem::take(&mut *diagnostic_joins().lock().unwrap());
    let joined = tokio::time::timeout(Duration::from_secs(3), async {
        for join in joins { let outcome = join.await; eprintln!("DIAGNOSTIC_WORKER_JOIN {outcome:?}"); }
    }).await;
    eprintln!("DIAGNOSTIC_JOIN_RESULT {joined:?}");
    result
}
...
async fn http_get_with_timeout(
    ...
    let join = tokio::task::spawn_blocking(move || {
        let result = http_get_blocking(addr, &host, &path, request_timeout);
        eprintln!("WIRE_RESULT host={host} result={result:?}");
        let _ = sender.send(result);
    });
    diagnostic_joins().lock().unwrap().push(join);
    receiver.await.map_err(|error| format!("HTTP request task failed: {error}"))?
}
...
fn http_get_blocking(
    ...
    eprintln!("WIRE_START host={host} unix_ms={} timeout={request_timeout:?}", SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis());
    let mut stream = TcpStream::connect_timeout(&addr, Duration::from_secs(5))?;
    eprintln!("WIRE_CONNECT host={host} elapsed={:?}", wire_start.elapsed());
    ...
    stream.flush()?;
    eprintln!("WIRE_SENT host={host} elapsed={:?}", wire_start.elapsed());
    let mut stream = DiagnosticReader { stream, host: host.to_owned(), start: wire_start, bytes: Vec::new(), headers: false, framed: false };
...
fn diagnostic_joins() -> &'static std::sync::Mutex<Vec<tokio::task::JoinHandle<()>>> {
    static JOINS: std::sync::OnceLock<std::sync::Mutex<Vec<tokio::task::JoinHandle<()>>>> = std::sync::OnceLock::new();
    JOINS.get_or_init(Default::default)
}
struct DiagnosticReader { stream: TcpStream, host: String, start: std::time::Instant, bytes: Vec<u8>, headers: bool, framed: bool }
impl Read for DiagnosticReader {
    fn read(&mut self, dst: &mut [u8]) -> std::io::Result<usize> {
        match self.stream.read(dst) {
            Ok(n) => {
                if self.bytes.is_empty() && n > 0 { eprintln!("WIRE_FIRST_BYTE host={} elapsed={:?}", self.host, self.start.elapsed()); }
                ...
                if let Some(end) = self.bytes.windows(4).position(|x| x == b"\r\n\r\n") {
                    ...
                    if !self.headers { eprintln!("WIRE_HEADERS host={} elapsed={:?} headers={head:?}", self.host, self.start.elapsed()); self.headers = true; }
                    let length = head.lines().find_map(|line| { let (key, value) = line.split_once(':')?; if key.eq_ignore_ascii_case("content-length") { value.trim().parse::<usize>().ok() } else { None } });
                    if !self.framed && length.is_some_and(|length| self.bytes.len() >= end + 4 + length) { eprintln!("WIRE_FRAMED_COMPLETE host={} elapsed={:?} bytes={}", self.host, self.start.elapsed(), self.bytes.len()); self.framed = true; }
                }
                if n == 0 { eprintln!("WIRE_EOF host={} elapsed={:?} bytes={}", self.host, self.start.elapsed(), self.bytes.len()); }
                Ok(n)
            }
            Err(error) => { eprintln!("WIRE_ERROR host={} elapsed={:?} kind={:?} bytes={} framed={}", self.host, self.start.elapsed(), error.kind(), self.bytes.len(), self.framed); Err(error) }
        }
    }
}
