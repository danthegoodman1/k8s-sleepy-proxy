...
NS='sleepypods-e2e-lifecycle-final'
...
def kube():
 row=run(['get','deployment,statefulset,replicaset,pod,service,persistentvolumeclaim','-l','sleepypods.io/instance-id','-o','json'])
 if row.get('exit_code')==0:
...
def capture(name,fn):
 with (OUT/name).open('w') as f:
  ...
  while not STOP.is_set() and time.monotonic()-started<1200:
   try:result=fn()
