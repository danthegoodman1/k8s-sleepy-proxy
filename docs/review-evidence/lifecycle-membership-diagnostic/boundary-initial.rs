...
async fn exercise(pg: PostgresStore, raw: &tokio_postgres::Client) -> TestResult {
    ...
    let result = tokio::time::timeout(Duration::from_secs(95), async {
        ...
        require(at_sixty["state"] == "deleting" && at_sixty["instance_state"] == "deleting" && at_sixty["generation"] == 3, "original 60s observation should still see exact Deleting3")?;
        ...
        let final_state = snapshot(raw).await?;
        eprintln!("CLEANUP_COMPLETE {final_state}");
        require(final_state["state"] == "deleted" && final_state["instance_state"].is_null(), "cleanup did not finalize deletion")?;
