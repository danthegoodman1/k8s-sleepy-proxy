...
echo "==> Reusing frozen final 6I image tags already in kind; no build, pull, or load"
...
start_port_forwards
frontline_capture_ip="$(KUBECONFIG="${kubeconfig}" kubectl -n "${namespace}" get pods -l app.kubernetes.io/name=sleepypods-frontline -o jsonpath='{.items[0].status.podIP}')"
docker exec -i -e "SNI_CAPTURE_FRONTLINE_IP=${frontline_capture_ip}" sleepypods-review-remediation-control-plane python3 -u - < "${repo_root}/.generated/phase6i-sni-before-network.py" > "${repo_root}/.generated/phase6i-sni-before-network.log" 2>&1 &
capture_pid=$!
for capture_wait in {1..100}; do
  if grep -q 'capture ready' "${repo_root}/.generated/phase6i-sni-before-network.log"; then break; fi
  sleep 0.1
done
grep -q 'capture ready' "${repo_root}/.generated/phase6i-sni-before-network.log"
echo "==> Capturing scoped frontend connection headers/DNS before cold requests"
