...
CREATE TABLE materialization_object_reservations (
    ...
    name text NOT NULL,
    materialization_id text NOT NULL REFERENCES materializations ON DELETE CASCADE,
...
EXPLAIN(ANALYZE,BUFFERS) 
    WITH selected AS (
        ...
        FROM route_bindings
        WHERE identity_kind = 'http'
            AND ((host_kind = 'exact' AND host = 'app-90000.example.com')
                OR (host_kind = 'wildcard_suffix' AND host = ANY(ARRAY['example.com','com']::text[])))
            AND (path_prefix IS NULL OR ('/api/users'::text::text IS NOT NULL AND (
                path_prefix = '/' OR path_prefix = '/api/users'::text
                OR (right(path_prefix, 1) = '/' AND starts_with('/api/users'::text, path_prefix))
                OR starts_with('/api/users'::text, path_prefix || '/')
            )))
    ...
    LEFT JOIN LATERAL (
        ...
        WHERE instance_id = selected.instance_id
            ...
            AND cluster_id = 'cluster' AND namespace = 'namespace' AND state = 'ready'
        LIMIT 1
    ) ready ON true
;
EXPLAIN(ANALYZE,BUFFERS) SELECT materialization_id FROM materialization_object_reservations WHERE cluster_id='cluster' AND api_group='' AND kind='Service' AND namespace='namespace' AND name='unrelated-new';
EXPLAIN(ANALYZE,BUFFERS) SELECT materialization_id FROM materialization_key_reservations WHERE cluster_id='cluster' AND namespace='namespace' AND key_name='logical' AND key_value='unrelated-new';
SELECT count(*) AS object_reservations FROM materialization_object_reservations;
SELECT count(*) AS key_reservations FROM materialization_key_reservations;
