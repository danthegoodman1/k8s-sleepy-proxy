...
async fn missing_pvc_binding_fails_materialization_without_ready_backend(
    ...
    wait_for_capacity_mismatch_inventory(
        kube.clone(),
        &config.namespace,
...
async fn wait_for_capacity_mismatch_inventory(
    kube: Client,
    namespace: &str,
    generation: u64,
) -> TestResult<()> {
    ...
    loop {
        if let (Some(pv), Some(pvc)) = (
            ...
            let claim = pv_spec
                .claim_ref
                .as_ref()
                .ok_or("PV is missing exact claimRef")?;
            if pv_spec.persistent_volume_reclaim_policy.as_deref() != Some("Retain")
                || pv_spec
                    .capacity
                    .as_ref()
            ...
            {
                return Err(
                    "live PV/PVC do not match the exact retained 1Mi/2Mi mismatch inventory".into(),
                );
            }
            ...
            if phase == Some("Bound") {
                return Err("capacity-mismatched PVC unexpectedly became Bound".into());
            }
        }
        if Instant::now() >= deadline {
            return Err(
                "did not observe real capacity-mismatched Pending PVC before the wake deadline"
                    .into(),
            );
        }
    ...
    [
        json!({
            ...
            "spec": {
                ...
                "claimRef": { "namespace": namespace, "name": UNBOUND_FIRST_PVC_NAME },
