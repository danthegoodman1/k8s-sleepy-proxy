...
namespace="sleepypods-e2e-binding-uid-20260908"
...
# Add-only experiment: no cluster lifecycle, no replacement, no Kubernetes
# cleanup. The fixture's own lifecycle operations affect only the objects it
# creates through its fresh isolated control-plane database.
test "$(KUBECONFIG="${kubeconfig}" kubectl config current-context)" = "kind-sleepypods-review-remediation"
test -z "$(KUBECONFIG="${kubeconfig}" kubectl get namespace "${namespace}" --ignore-not-found -o name)"
test -z "$(KUBECONFIG="${kubeconfig}" kubectl get clusterrole "sleepypods-control-plane-${namespace}" --ignore-not-found -o name)"
test -z "$(KUBECONFIG="${kubeconfig}" kubectl get clusterrolebinding "sleepypods-control-plane-${namespace}" --ignore-not-found -o name)"
for experiment_pv_name in failure-unbound-pv failure-invalid-volume-pv; do
  test -z "$(KUBECONFIG="${kubeconfig}" kubectl get pv "${experiment_pv_name}" --ignore-not-found -o name)"
done
echo "==> Add-only stale-claim experiment: existing frozen images, new namespace ${namespace}"
