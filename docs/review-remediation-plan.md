...
| --- | --- | --- | --- |
| Incomplete | Work | 1A: Fix stale wake-error test assertion | Missing: passing `proxy_api_transport` run. |
| Incomplete | Work | 1B: Remove `to_string` shadow; clear clippy warnings | Missing: clean clippy output. |
| Incomplete | Work | 1C: CI workflow with fmt/clippy/test/postgres gates | Missing: workflow file and green CI run. |
| Incomplete | Gate | Default branch green under CI | Missing: CI run link. |
| Incomplete | Test | Postgres conformance runs (not skipped) in CI | Missing: CI log showing suite executed. |
...
| Status | Type | Item | Evidence / Gap |
| --- | --- | --- | --- |
| Incomplete | Work | 2A: Pooled http1/http2 clients in `HttpProxy` | Missing: implementation and smoke deltas. |
| Incomplete | Work | 2B: Sidecar keep-alive re-enabled | Missing: change and connection-reuse test. |
| Incomplete | Work | 2C: Chunked header sniffing with deadline | Missing: implementation and protocol tests. |
| Incomplete | Work | 2D: Frontline connection-task reaping | Missing: change in three accept loops. |
| Incomplete | Work | 2E: Per-permit metrics off the stderr sink | Missing: implementation. |
| Incomplete | Work | 2F: `[workspace.dependencies]` and release profile | Missing: manifest changes. |
| Incomplete | Gate | Strict-budget smokes pass post-change | Missing: smoke output. |
| Incomplete | Test | Before/after smoke comparison recorded | Missing: measurements. |
...
| Status | Type | Item | Evidence / Gap |
| --- | --- | --- | --- |
| Incomplete | Work | 3A: Lifecycle invalidations through the broker | Missing: broker plumbed into sidecar/reconciler/delete plus tests. |
| Incomplete | Work | 3B: Raise positive route TTL after 3A | Missing: TTL change with invalidation coverage. |
| Incomplete | Work | 3C: Reconciler-driven sleep cleanup with drain grace | Missing: `idle.rs` no longer deletes objects in-RPC. |
| Incomplete | Work | 3D: Two-phase delete with Deleting CAS | Missing: store guard and interleaving test. |
| Incomplete | Work | 3E: Hash-based instance name qualifier | Missing: `kubernetes_name.rs` change and collision test. |
| Incomplete | Work | 3F: Idle re-arm, real active count, RTT abort | Missing: sidecar idle changes and tests. |
| Incomplete | Work | 3G: Stream timeouts and TCP keepalive | Missing: proxy-core timeout knobs and tests. |
| Incomplete | Work | 3H: Sidecar token via Secret | Missing: manifest render change. |
| Incomplete | Gate | Kind e2e proves invalidation-driven sleep/delete | Missing: e2e run output. |
| Incomplete | Test | Cancellation/interleaving unit coverage listed above | Missing: named tests. |
