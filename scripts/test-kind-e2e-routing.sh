...
cluster_name="${SLEEPYPODS_KIND_CLUSTER:-sleepypods-e2e-routing-test}"
namespace="${SLEEPYPODS_KIND_E2E_NAMESPACE:-sleepypods-e2e-routing}"
...
image_tag="${SLEEPYPODS_IMAGE_TAG:-kind-e2e-routing}"
...
operator_port="${SLEEPYPODS_KIND_E2E_OPERATOR_PORT:-19251}"
frontline_port="${SLEEPYPODS_KIND_E2E_FRONTLINE_PORT:-19280}"
kubeconfig="$(mktemp)"
created_cluster=0
control_plane_pf=""
frontline_pf=""
...
require_command() {
  ...
  if ! command -v "${name}" >/dev/null 2>&1; then
    echo "${name} is required for the routing kind E2E" >&2
    exit 127
  fi
}
...
KUBECONFIG="${kubeconfig}" kubectl label namespace "${namespace}" \
  sleepypods.io/kind-e2e=routing --overwrite
...
metadata:
  ...
  labels:
    ...
    sleepypods.io/kind-e2e: routing
spec:
  replicas: 1
  selector:
    matchLabels:
...
KUBECONFIG="${kubeconfig}" kubectl -n "${namespace}" port-forward \
  svc/sleepypods-control-plane "${operator_port}:50051" >/tmp/sleepypods-control-plane-routing-port-forward.log 2>&1 &
control_plane_pf=$!
KUBECONFIG="${kubeconfig}" kubectl -n "${namespace}" port-forward \
  svc/sleepypods-frontline "${frontline_port}:8080" >/tmp/sleepypods-frontline-routing-port-forward.log 2>&1 &
frontline_pf=$!

echo "==> Running routing kind E2E driver"
KUBECONFIG="${kubeconfig}" \
  SLEEPYPODS_KIND_E2E_ROUTING=1 \
  ...
  cargo test -p control-plane --test kind_e2e_routing -- --ignored --nocapture

echo "routing full-platform kind E2E completed"
