...
cluster_name="${SLEEPYPODS_KIND_CLUSTER:-sleepypods-e2e-restart-test}"
namespace="${SLEEPYPODS_KIND_E2E_NAMESPACE:-sleepypods-e2e-restart}"
...
require_command() {
  ...
  if ! command -v "${name}" >/dev/null 2>&1; then
    echo "${name} is required for the restart kind E2E" >&2
    exit 127
  fi
}
...
KUBECONFIG="${kubeconfig}" kubectl label namespace "${namespace}" \
  sleepypods.io/kind-e2e=restart --overwrite
