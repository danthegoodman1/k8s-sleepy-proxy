...
image_tag="${SLEEPYPODS_IMAGE_TAG:-kind-e2e-stateless}"
...
require_command() {
  ...
  if ! command -v "${name}" >/dev/null 2>&1; then
    echo "${name} is required for the stateless kind E2E" >&2
    exit 127
  fi
}
...
KUBECONFIG="${kubeconfig}" kubectl label namespace "${namespace}" \
  sleepypods.io/kind-e2e=stateless --overwrite
